...
def _downsample_timeseries(data, key_fields=None, max_points=24):
    ...
    Looks for lists of dicts with a timestamp-like key and numeric values.
    Splits into max_points buckets, averages numeric fields, keeps first
...
class GarminExporter:
    ...
    def run(self):
        now = datetime.now()
        suffix = "_compact" if _compact_mode else ""
    ...
    def _write_split(self, full_text: str, base_filename: str) -> list:
        ...
        for i, file_sections in enumerate(files, 1):
            ...
            suffix = f"_part{i}of{total}"
            fname = base_filename.replace("_compact.txt", f"_compact{suffix}.txt")
...
""",
    ...
    global _limiter, _compact_mode, _split_mode
    _limiter = RateLimiter(base_delay=args.delay)
    _split_mode = args.split
    if _split_mode:
        args.compact = True  # --split implies --compact
